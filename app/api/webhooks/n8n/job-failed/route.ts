...
import { processJobFailed } from '@/lib/services/webhooks';
import { jobFailedWebhookSchema } from '@/lib/validators/webhooks';
...
export async function POST(request: Request) {
  try {
    ...
    const payload = jobFailedWebhookSchema.parse(json);
    const result = await processJobFailed(payload);
    return ok(result);
  } catch (error) {
    return fail(error);
  }
