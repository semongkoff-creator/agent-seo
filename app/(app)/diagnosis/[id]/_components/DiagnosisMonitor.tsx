...
const narration = {
  ...
  id: [
    'Memeriksa crawlability, indexability, dan struktur situs.',
    'Membandingkan cakupan intent dengan sinyal ranking saat ini.',
    'Mengestimasi gap authority terhadap domain kompetitor.',
    'Mengevaluasi friksi konversi di landing page organik.',
    'Menyiapkan payload diagnosis terstruktur.',
    'Menunggu workflow latar belakang selesai.'
  ]
} as const;

export function DiagnosisMonitor({ diagnosisId, projectName, locale }: DiagnosisMonitorProps) {
  ...
  useEffect(() => {
    const ticker = window.setInterval(() => {
      setMessageIndex((current) => (current + 1) % narration[locale].length);
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-6 px-4 py-10 md:px-6 lg:px-8">
      <div className="rounded-[28px] border border-outline-variant bg-surface-container-lowest p-6 shadow-sm md:p-8">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.26em] text-primary">{copy.diagnosisRunning}</p>
            ...
            <p className="mt-2 text-sm leading-6 text-on-surface-variant md:text-base">
              {locale === 'id'
                ? 'Analisis masih diproses di latar belakang. Kami membaca status terbaru dari Supabase dan akan memuat ulang begitu hasil tersimpan.'
                : 'The analysis is still processing in the background. We read the latest status from Supabase and refresh as soon as the result is stored.'}
            </p>
          </div>
        ...
        <div className="mt-6 rounded-2xl border border-outline-variant bg-white p-4">
          <div className="flex items-center justify-between text-sm">
            <span className="font-semibold text-on-surface">{locale === 'id' ? 'Progress' : 'Progress'}</span>
          ...
          <p className="mt-3 text-sm text-on-surface-variant">{narration[locale][messageIndex]}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        ...
        <div className="rounded-[28px] border border-outline-variant bg-surface-container-lowest p-5 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.26em] text-on-surface-variant">{copy.checksLabel}</p>
          <ul className="mt-3 space-y-2 text-sm text-on-surface-variant">
            <li className="flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-primary" />
              {locale === 'id' ? 'Ketercrawlan dan indexing' : 'Crawlability and indexing'}
            </li>
            <li className="flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-primary" />
              {locale === 'id' ? 'Kesenjangan relevance dan authority' : 'Relevance and authority gaps'}
            </li>
            <li className="flex items-center gap-2">
              <Sparkles className="h-4 w-4 text-primary" />
              {locale === 'id' ? 'Sinyal friksi konversi' : 'Conversion friction signals'}
            </li>
          </ul>
        </div>
      </div>
      ...
      {error ? (
        <div className="rounded-[28px] border border-error/30 bg-error-container/20 p-5 text-sm text-on-error-container">
          <p className="font-semibold">{copy.monitoringIssue}</p>
