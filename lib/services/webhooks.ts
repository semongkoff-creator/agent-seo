...
export async function processDiagnosisComplete(payload: DiagnosisCompleteWebhookInput) {
  ...
  const { error: diagnosisError } = await db
    ...
    .update({
      ...
      technical_health_score: payload.result.technical_health_score ?? null,
      ai_visibility_score: payload.result.ai_visibility_score ?? null,
      technical_section: payload.result.technical_section ?? {},
      keyword_section: payload.result.keyword_section ?? {},
      ai_overview_section: payload.result.ai_overview_section ?? {},
      business_impact_section: payload.result.business_impact_section ?? {},
...
export async function processObjectiveComplete(payload: ObjectiveCompleteWebhookInput) {
  ...
  const { error: objectiveError } = await db
    ...
    .update({
      ...
      pillar: payload.result.pillar ?? null,
      checklist_summary: payload.result.checklist_summary ?? null,
      estimated_completion: payload.result.estimated_completion ?? null,
      linked_technical_errors: payload.result.linked_technical_errors ?? [],
      action_items: payload.result.action_items ?? [],
      target_metrics: payload.result.target_metrics ?? {},
      roi_estimate: payload.result.roi_estimate ?? null,
