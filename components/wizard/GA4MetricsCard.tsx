  ...
  return (
    <section className="rounded-[24px] border border-outline-variant bg-surface-container-lowest p-5 shadow-sm">
      <div className="flex items-center justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-on-surface-variant">Traffic Trend</p>
          <h3 className="mt-1 text-lg font-semibold text-on-surface">GA4 mock metrics</h3>
        </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
        <MetricBox label="Session" value={formatCount(data.session.value)} trend={data.session.trendPct} />
        <MetricBox label="Page View" value={formatCount(data.pageView.value)} trend={data.pageView.trendPct} />
        <MetricBox
