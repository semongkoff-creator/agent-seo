...
export function CrawlErrorsCard({
  projectId,
  value,
  onChange
}: {
  ...
  async function loadLatest() {
    ...
    if (response.ok) {
      ...
      if (typeof body.data?.crawlErrorsTotal === 'number') {
        onChange?.(body.data.crawlErrorsTotal);
      }
    }
  }
  ...
  async function syncNow() {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setSummary(next);
        if (typeof next?.crawlErrorsTotal === 'number') {
          onChange?.(next.crawlErrorsTotal);
        }
      }
    } finally {
      setSyncing(false);
  ...
  const breakdown = summary?.crawlErrorsBreakdown ?? {};
  ...
  return (
    <section className="flex h-full flex-col rounded-[24px] border border-outline-variant bg-white p-4 shadow-sm">
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.24em] text-on-surface-variant">Crawl Errors</p>
      ...
      <div className="mt-4 flex-1 space-y-2 rounded-2xl bg-surface-container-low p-3">
        {topBreakdown.length > 0 ? (
          ...
          <p className="text-sm text-on-surface-variant">Use GSC sync to populate crawl error samples.</p>
        )}
      </div>
